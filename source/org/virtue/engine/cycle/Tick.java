...
public abstract class Tick {
	...
	/**
	 * Represents if this {@link Tick} needs to be destroyed in the next game
	 * cycle.
	 */
		...
		if (paused == true)
			try {
				throw new IllegalAccessException(
						"The requested tick is currently paused, therefore cannot be paused again.");
	...
	public void resume() {
		if (paused == false)
			try {
				throw new IllegalAccessException(
						"The requested tick is not currently paused, therefore cannot be resumed.");
			} catch (IllegalAccessException e) {
				e.printStackTrace();
			}
		paused = false;
	 ...
	 * 
	 * @param skipCount
	 *            The amount of upcoming game cycles to skip.
	 */
