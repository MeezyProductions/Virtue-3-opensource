...
public class GameEngine implements Runnable {
	...
	private static final Logger logger = LoggerFactory
			.getLogger(OnDemandSession.class);

	/**
	 * Represents the {@link TickService} responsible for performing game
	 * cycles.
	 */
	private final TickService TICK_SERVICE = new TickService();

	/**
	 * Represents the {@link ExecutorService} responsible for providing threads
	 * used for our network filter.
	 */
	private final NetworkThreadPool NETWORK_POOL = new NetworkThreadPool();

	/**
	 * Represents the {@link ExecutorService} responsible for providing threads
	 * used for our bound channel.
	 */
	 ...
	 * used for our network filter.
	 */
	private final ExecutorService WORKER_EXECUTOR = Executors
			.newCachedThreadPool(new WorkerThreadPool());

	/**
	 * Represents the {@link ExecutorService} responsible for providing threads
	 * to the {@link UpdateService}
	 */
	private final ExecutorService SERVICE_EXECUTOR = Executors
			.newCachedThreadPool(new ServiceThreadFactory());
		...
		while (state != null && state.equals(WorkerState.LIVE)) {
			...
			if (GameClock.CYCLE_RATE > elapsedCycleTime) {
				try {
					...
					logger.error("Caught Engine Exception on Cycle: "
							+ cycleCount, e);
				}
			} else {
	...
	public int getAvailableWorkerCount() {
		...
		int workerCount = availableProcessors >= 6 ? availableProcessors
